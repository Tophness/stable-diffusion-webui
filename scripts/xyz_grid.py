...
def draw_xy_grid(p, xs, ys, zs, x_labels, y_labels, z_labels, cell, draw_grid, draw_legend, include_lone_images):
    ...
    for iz, z in enumerate(zs):
        for iy, y in enumerate(ys):
            for ix, x in enumerate(xs):
                state.job = f"{ix + iy + iz * len(xs) + 1} out of {len(xs) * len(ys) * len(zs)}"
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        with gr.Row():
            z_type = gr.Dropdown(label="Z type", choices=[x.label for x in current_axis_options], value=current_axis_options[2].label, visible=False, type="index", elem_id="z_type")
            z_values = gr.Textbox(label="Z values", visible=False, lines=1)
        
        draw_grid = gr.Checkbox(label='Draw grid', value=False)
        draw_legend = gr.Checkbox(label='Draw legend', value=True)
        include_lone_images = gr.Checkbox(label='Include Separate Images', value=False)
        no_fixed_seeds = gr.Checkbox(label='Keep -1 for seeds', value=False)

        return [x_type, x_values, y_type, y_values, z_type, z_values, draw_grid, draw_legend, include_lone_images, no_fixed_seeds]

    def run(self, p, x_type, x_values, y_type, y_values, z_type, z_values, draw_grid, draw_legend, include_lone_images, no_fixed_seeds):
        ...
        if x_opt.label == 'Steps':
            ...
            total_steps = sum(zs) * len(xs) * len(zs)
        else:
